...
/** @brief Use any ID above 200 for the object's ID number.  
The selection scheme takes care of keeping the population up-to-date - we only have to worry about the selection method itself.  
This selector picks randomly, but only from the upper half of the population (based on the scaled fitness scores). 
*/
...
public:
    ...
    FinickySelector(const FinickySelector& orig)
    {
        copy(orig);
    }
...
/** @brief Restricted mating algorithm.
This is the genetic algorithm that does the restricted mating.    
It is similar to the steady state genetic algorithm, 
but we modify the selection part of the step method to do the restricted mating.
*/
