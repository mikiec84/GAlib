/** 
  @file GA1DBinStrGenome.h
  @brief defines the interface for the 1D binary string genome.
...
/** @brief 1D Binary String Genome.
 ...
 @li @b ==, !=
  ...
  @b contents of the genome, @b NOT the @b behaviour.  So as long as the bitstreams
...
public:
    ...
    GA1DBinaryStringGenome& operator=(const short array []) ///< operator= no err checks!
    {
    ...
    GA1DBinaryStringGenome& operator=(const int array []) ///< operator= no err checks!
    {
        for(unsigned int i = 0; i < sz; i++)
        {
            gene(i, *(array + i));
