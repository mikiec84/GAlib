...
out, please turn off the lights).
  ...
  @todo binary to integer phenotype?

  @todo Eventually we may want to cache the values in the phenotype mapping, 
        but for now we'll generate them on the fly.
  @todo Need to write a read method that can interpret binary/decimal input.
 */
...
public:
    ...
    GABin2DecPhenotype * clone() const
    {
    ...
    unsigned int nPhenotypes() const
    {
        return core->n;
    }
