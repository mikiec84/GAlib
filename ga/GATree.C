...
/** @brief GATree destructor.
The destructor just goes through the tree and deletes every node.  
As with any method that uses the BASE tree, we have to use its members so it doesn't get messed up.
*/
template <class T>
GATree<T>::~GATree()
{
...
/** @brief make a copy of a tree.
 @bug  Yes, this is really ugly.  We do a complete destruction of the existing tree then we copy the new one.  
No caching, no nothing.  Oh well.  The iterator is set to the root node - it should be set to the corresponding node, but
...
/** @brief Swap two subtrees.  
 We use the nodes pointed to by the iterators in the
...
/** @brief  Swap two subtrees using node indices. 
 Same as the swaptree above, but this routine uses the node indices to do
 the swap.  This can be @b dangerous:  if one of the nodes is a decendent of the
...
/** @brief Swap two nodes in a tree.
 Swap two nodes in a tree, leave their subtrees intact.  
 This routine does not affect the iterator or the size or depth of the tree.
*/
template <class T> 
...
/** @brief Recursive routines for the Tree objects.
...
/** @brief destroys a sub-tree specified by a node.
