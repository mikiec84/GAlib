...
/** @brief This object contains a set of alleles for use with a similarly typed genome.
...
complex, i.e. it is @b not OK to do a bit-copy of each object.  We should do
...
discretized set, etc)? <br>  
- I wanted to be able to have an array that uses sets of many different types.  
I suppose the allele set should be completely polymorphic like the rest of 
the GAlib objects, but for now we'll do it as a single object with multiple 
personalities (and a state).
...
- should the assignment operator check to be sure that no allele is duplicated, 
or is it OK to have duplicate alleles in a set?  <br>
- For now we allow duplicates (via either the add or assignemnt ops).
 */
...
public:
    ...
    {
        return new GAAlleleSet<T>(*this);
    }
    void link(GAAlleleSet<T>& set);
    void unlink();
    int size() const
    {
        return core->sz;   /// only meaninful for enumerated sets
    }
    ...
    T lower() const
    {
        return core->a[0];   /// only for bounded sets
    }
    T upper() const
    {
        return core->a[1];
