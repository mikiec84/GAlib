/** @file GAListBASE.h
 ...
 @todo  Probably should put the size (and depth for trees) into the templateized
...
/** @brief base list class from which template lists are derived.
...
do this, you'll get a copy of the list object, but not a duplicate of the list.
 ...
 @li insert
  ...
  for using the insert method looks like this:
    ...
    @c   mylist.insert(newnode, oldnode, GAListBASE::before);
    @c   mylist.insert(newnode, oldnode);
...
private:
    ...
    GAListBASE & operator=(const GAListBASE &)
    {
        return *this;   /// or op=
    }
...
/** @brief base class for iterators for the list objects.
...
will result in a no-op, and NULL will be returned.
 ...
 @li current, head, tail, next, prev, warp
...
public:
    ...
    GANodeBASE * head()
    {
        return(list ? (node = list->hd) : (GANodeBASE *)0);
    }
    GANodeBASE * tail()
    {
