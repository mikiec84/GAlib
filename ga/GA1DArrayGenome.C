...
/** the pointer return should be deleted somewhere.*/
template <class T> 
...
template <class T>
GA1DArrayAlleleGenome<T>::GA1DArrayAlleleGenome(unsigned int length, const GAAlleleSet<T> & s, GAGenome::Evaluator f, void * u) :
...
template <class ARRAY_TYPE> 
float GA1DArrayGenome<ARRAY_TYPE>::
...
template <class T> int
GA1DArrayGenome<T>::OnePointCrossover(const GAGenome& p1, const GAGenome& p2,
                     GAGenome* c1, GAGenome* c2)
{
    const GA1DArrayGenome<T> &mom = DYN_CAST(const GA1DArrayGenome<T> &, p1);
    const GA1DArrayGenome<T> &dad = DYN_CAST(const GA1DArrayGenome<T> &, p2);
