/** 
	@file GA1DArrayGenome.h
    @brief This header defines the interface for the 1D array genome.
...
I'll try to do that for common instantiations (float, char).

The objects in the array must have the following operators defined: <br>
 @b =  @b ==  @b != <br>
 @b >> must be defined if you use the default read methods.

 @todo
 @li If you want speed, specialize the comparison routines and copy routines
    so that you can use memcpy, memmove, memcmp rather than looping through
    each element.
 @li make the object defined for simple types, if you want to use complex types
...
public:
    ...
    GA1DArrayGenome<T>& operator=(const T array []) ///< operator= no err checks!
    {
...
public:
    ...
    GA1DArrayAlleleGenome<T>& operator=(const T array []) ///< operator= no err checks!
    {
